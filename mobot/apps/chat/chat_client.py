...
from mobot.apps.chat.context import MessageContextFactory, MobotContext
from .handlers import *
from .handlers.utils import regex_filter, drop_session_state_filter, chat_session_state_filter
...
class MobotHandler:
    ...
    @property
    def should_run(self, ctx: MobotContext) -> bool:
        return all(condition(ctx) for condition in self._conditions) if self._conditions else True
...
class Mobot:
    ...
    def get_context_from_message(self, message: SignalMessage) -> MobotContext:
        context: MobotContext = self.message_context_factory.get_message_context(message)
        return context
    ...
    @staticmethod
    def _regex_to_filter_func(regex: str) -> Callable[[MobotContext], bool]:
    ...
    def register_handler(self,
        ...
        Args:
            conditions: A bunch of lambdas to evaluate whether or not this should run.
        ...
        handler = MobotHandler(method=method, conditions=conditions, order=order, chat_session_states=chat_session_states,
                               drop_session_states=drop_session_states)
    ...
    def register_handlers(self):
        self.register_handler(conditions=[regex_filter("^(u|unsubscribe)$")],
                              method=unsubscribe_handler)
        self.register_handler(conditions=[regex_filter("^(s|subscribe)$")],
                              method=subscribe_handler)
        self.register_handler(conditions=[regex_filter(""), ],
                              method=handle_greet_customer,
                              chat_session_states={MobotChatSession.State.NOT_GREETED},
                              order=1)  # First, say hello to the customer
        self.register_handler(conditions=[regex_filter("")],
                              method=handle_start_conversation,
                              chat_session_states={MobotChatSession.State.NOT_GREETED},
                              order=2)  # Then, handle setting up drop session
        self.register_handler(conditions=[regex_filter("")],
                              method=handle_already_greeted,
                              chat_session_states={MobotChatSession.State.INTRODUCTION_GIVEN})
        self.register_handler(conditions=[regex_filter(""),
                                          drop_session_state_filter({DropSession.State.EXPIRED})],
                              method=handle_drop_expired)
        self.register_handler(conditions=[regex_filter("")],
                              method=handle_drop_not_ready,
                              drop_session_states={DropSession.State.NOT_READY})
        self.register_handler(conditions=[regex_filter("")],
                              method=handle_no_handler_found)
        self.register_handler(conditions=[regex_filter("^p$")],
                              method=privacy_policy_handler)
        self.register_handler(conditions=[regex_filter("^(i|inventory)$")],
                              method=inventory_handler,
